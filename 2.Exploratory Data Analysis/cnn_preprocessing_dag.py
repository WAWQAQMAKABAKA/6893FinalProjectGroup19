...
file_path = os.path.join(os.path.dirname(__file__), "cnn.csv")
...
def read_csv(**kwargs):
    ...
    '''
    cnn = pd.read_csv(file_path, encoding='utf-8')
    kwargs['ti'].xcom_push(key='cnn_data', value=cnn.to_dict())
...
def clean_data(**kwargs):
    ...
    '''
    cnn = pd.DataFrame(kwargs['ti'].xcom_pull(key='cnn_data', task_ids='read_csv'))
    cnn['body'] = cnn['body'].apply(remove_unnecessary_content)
    kwargs['ti'].xcom_push(key='cleaned_data', value=cnn.to_dict())
...
def tokenize_and_filter(**kwargs):
    ...
    '''
    cnn = pd.DataFrame(kwargs['ti'].xcom_pull(key='cleaned_data', task_ids='clean_data'))
    ...
    cnn['processed_body'] = cnn['body'].apply(preprocess_text)
    kwargs['ti'].xcom_push(key='processed_data', value=cnn.to_dict())
...
def save_to_csv(**kwargs):
    ...
    please change to the correct one, for example(mainstreamsource['title'])
    '''
    cnn = pd.DataFrame(kwargs['ti'].xcom_pull(key='processed_data', task_ids='tokenize_and_filter'))
    output_path = os.path.join(os.path.dirname(__file__), "cnn_processed.csv")
    cnn.to_csv(output_path, index=False, encoding='utf-8')

# define the DAG
with DAG(
    dag_id='cnn_preprocessing_dag',
